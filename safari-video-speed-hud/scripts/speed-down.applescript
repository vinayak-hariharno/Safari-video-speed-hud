    ...
    if (count of windows) > 0 then
        tell front document
            ...
            (function() {
                ...
                let prevSpeed = speeds.slice().reverse().find(s => s < currentRate);
                if (prevSpeed === undefined) prevSpeed = speeds[0];

                videos.forEach(v => { v.playbackRate = prevSpeed; });
                ...
                badge.textContent = '⚡ ' + prevSpeed.toFixed(2).replace(/\\.00$/, '.0').replace(/0$/, '') + 'x';
